...
namespace BreadTh.StronglyApied.Direct.Core
{
    ...
    {
        public (bool success, JObject result) TryTokenize(string input)
        {
            ...
            try
            {
                return (true, JObject.Parse(input));
            }
            catch (Exception)
            {
                return (false, null);
        ...
        public (T result, List<ErrorDescription> errors) MapToModel<T>(
            JTokenWrapper rootToken, StronglyApiedObjectAttribute rootAttribute)
        {
            List<ErrorDescription> errors = new List<ErrorDescription>();
            T result = (T)MapObject(typeof(T), rootToken, "", rootAttribute);
            return (result, errors);

            dynamic DetermineTypeAndMap(FieldInfo fieldInfo, JTokenWrapper token, JTokenWrapper parentToken, string path)
            {
            ...
            dynamic MapObject(Type objectType, JTokenWrapper token, string path, StronglyApiedObjectAttribute attribute)
            {
                if(attribute == null)
            ...
            dynamic[] MapArray(FieldInfo fieldInfo, JTokenWrapper parentToken, string path)
            {
                ...
                {
                    List<JTokenWrapper> tokenAsArray = parentToken.GetChildren(fieldName).ToList();
            ...
            dynamic MapFieldInArray(FieldInfo fieldInfo, Type type, JTokenWrapper token, string path)
            {
            ...
            dynamic MapFieldInObject(FieldInfo fieldInfo, Type type, JTokenWrapper parentToken, string path)
            {
                StronglyApiedRelationBaseAttribute relationAttribute = fieldInfo.GetCustomAttribute<StronglyApiedRelationBaseAttribute>(true);
                string fieldName = relationAttribute?.name ?? fieldInfo.Name;
